...
class FirestoreSyncManagerTest {
    ...
    private val testRecord = DownloadRecord(
        id = 1L,
    ...
    @Test
    fun `processPendingOperations processes UPLOAD operations from queue`() = runTest(testDispatcher) {
        val uploadOp = SyncQueueEntity(
            id = 1L,
    ...
    @Test
    fun `processPendingOperations deletes queue entry on successful UPLOAD`() = runTest(testDispatcher) {
        val uploadOp = SyncQueueEntity(
            id = 1L,
            downloadId = testRecord.id,
            operation = "UPLOAD",
    ...
    @Test
    fun `processPendingOperations processes DELETE operations from queue`() = runTest(testDispatcher) {
        val deleteOp = SyncQueueEntity(
            id = 2L,
            downloadId = testRecord.id,
            operation = "DELETE",
            createdAt = System.currentTimeMillis(),
    ...
    @Test
    fun `syncNewRecord encrypts and uploads record`() = runTest(testDispatcher) {
    ...
    @Test
    fun `syncNewRecord triggers LRU eviction when at tier limit`() = runTest(testDispatcher) {
    ...
    @Test
    fun `queueDeletion inserts DELETE operation in sync queue`() = runTest(testDispatcher) {
    ...
    @Test
    fun `observeSyncStatus emits Idle initially`() = runTest(testDispatcher) {
