...
class FindExistingDownloadUseCaseTest {
    ...
    @Test
    fun `returns null when no completed downloads match`() =
        runTest {
    ...
    @Test
    fun `returns null when url does not match any completed record`() =
        runTest {
    ...
    @Test
    fun `returns null when matched record has no content uri`() =
        runTest {
            coEvery { repository.getCompletedSnapshot() } returns listOf(completedRecord)
    ...
    @Test
    fun `returns null when file is not accessible`() =
        runTest {
            coEvery { repository.getCompletedSnapshot() } returns listOf(completedRecord)
    ...
    @Test
    fun `returns ExistingDownload when url matches and file is accessible`() =
        runTest {
            coEvery { repository.getCompletedSnapshot() } returns listOf(completedRecord)
            coEvery { fileAccessManager.resolveContentUri(completedRecord) } returns "content://media/downloads/42"
    ...
    @Test
    fun `matches url despite tracking params in stored record`() =
        runTest {
            coEvery { repository.getCompletedSnapshot() } returns listOf(completedRecord)
            coEvery { fileAccessManager.resolveContentUri(completedRecord) } returns "content://media/downloads/42"
            coEvery { fileAccessManager.isFileAccessible("content://media/downloads/42") } returns true
    ...
    @Test
    fun `matches youtu_be short url against stored full url`() =
        runTest {
            coEvery { repository.getCompletedSnapshot() } returns listOf(completedRecord)
            coEvery { fileAccessManager.resolveContentUri(completedRecord) } returns "content://media/downloads/42"
            coEvery { fileAccessManager.isFileAccessible("content://media/downloads/42") } returns true
