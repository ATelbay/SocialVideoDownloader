...
class RestoreFromCloudUseCaseTest {
    ...
    private fun record(sourceUrl: String, createdAt: Long) = DownloadRecord(
        id = 0,
        sourceUrl = sourceUrl,
        videoTitle = "Title",
    ...
    @Test
    fun `fetches all cloud records and inserts non-duplicates into local DB`() = runTest {
    ...
    @Test
    fun `dedup - records with same sourceUrl AND createdAt are skipped`() = runTest {
    ...
    @Test
    fun `dedup - same sourceUrl but different createdAt is not a duplicate`() = runTest {
        val existing = record("https://example.com/video1", 1000L)
    ...
    @Test
    fun `returns RestoreResult with correct counts`() = runTest {
    ...
    @Test
    fun `when decryption fails (fetchAllRecords throws), returns error result`() = runTest {
    ...
    @Test
    fun `insert failure counts as failed`() = runTest {
        val cloudRecord = record("https://example.com/video1", 1000L)
        coEvery { cloudBackupRepository.fetchAllRecords() } returns listOf(cloudRecord)
        every { downloadRepository.getAll() } returns flowOf(emptyList())
