...
class ObserveCloudCapacityUseCaseTest {
    ...
    @Test
    fun `combines cloud record count with tier limit to emit capacity`() = runTest {
    ...
    @Test
    fun `isNearLimit is false when count is below 90 percent of limit`() = runTest {
    ...
    @Test
    fun `isNearLimit is true when count is exactly 90 percent of limit`() = runTest {
    ...
    @Test
    fun `isNearLimit is true when count is above 90 percent of limit`() = runTest {
    ...
    @Test
    fun `tier change updates the capacity info`() = runTest {
        coEvery { cloudBackupRepository.getCloudRecordCount() } returns 950
