...
class ObserveHistoryItemsUseCaseTest {
    ...
    @Test
    fun `maps DownloadRecord list to HistoryListItem list with all fields correctly mapped`() =
        runTest {
            val rec = record()
    ...
    @Test
    fun `resolves contentUri via HistoryFileManager resolveContentUri for each record`() =
        runTest {
    ...
    @Test
    fun `sets isFileAccessible=true when contentUri is non-null and fileManager isFileAccessible returns true`() =
        runTest {
            val rec = record()
            fileManager.resolveContentUriResult = { "content://media/external/video/1" }
            fileManager.isFileAccessibleResult = { true }
    ...
    @Test
    fun `sets isFileAccessible=false when contentUri is null`() =
        runTest {
            val rec = record()
    ...
    @Test
    fun `sets isFileAccessible=false when fileManager isFileAccessible returns false`() =
        runTest {
            val rec = record()
            fileManager.resolveContentUriResult = { "content://media/external/video/1" }
    ...
    @Test
    fun `emits updated list when repository flow emits new data`() =
        runTest {
            fileManager.resolveContentUriResult = { null }
            fileManager.isFileAccessibleResult = { false }
