...
class DeleteAllHistoryUseCaseTest {
    ...
    @Test
    fun `calls deleteAll on repository`() =
        runTest {
    ...
    @Test
    fun `skips file deletion and still calls deleteAll when deleteFiles is false`() =
        runTest {
            var deleteFileCallCount = 0
            fileManager.deleteFileResult = { _ ->
                deleteFileCallCount++
                true
            }
            fileManager.resolveContentUriResult = { r -> r.mediaStoreUri }

            repository.recordsFlow.emit(
                listOf(
    ...
    @Test
    fun `deletes files for all records that resolve to a contentUri`() =
        runTest {
    ...
    @Test
    fun `skips file deletion for records that resolve to null contentUri`() =
        runTest {
            var deleteFileCallCount = 0
            fileManager.deleteFileResult = { _ ->
                deleteFileCallCount++
    ...
    @Test
    fun `still calls repository deleteAll even when file deletion fails`() =
        runTest {
            fileManager.resolveContentUriResult = { r -> r.mediaStoreUri }
    ...
    @Test
    fun `reports file cleanup failures as a count of failed deletions`() =
        runTest {
            fileManager.resolveContentUriResult = { r -> r.mediaStoreUri }
    ...
    @Test
    fun `returns zero failedFileDeletions when all files deleted successfully`() =
        runTest {
            fileManager.resolveContentUriResult = { r -> r.mediaStoreUri }
    ...
    @Test
    fun `works correctly with empty history`() =
        runTest {
            repository.recordsFlow.emit(emptyList())
